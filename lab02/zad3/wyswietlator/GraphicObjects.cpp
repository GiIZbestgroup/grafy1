...
void GraphicObjects::createLines(int numberofNodes,std::vector<Point> &points,std::vector<std::vector<int>> &connectionArray,std::vector<sf::VertexArray> &lines,std::vector<int> vektor)
{
    ...
    for(int i=0;i<numberofNodes;i++)
    {
        for(int j=0;j<i;j++)
        {
             if(connectionArray[i][j])
            {
                ...
                {
                  std::cout<<vektor[k]<<" "<<i<<" "<<j<<std::endl;
                  if(vektor[k] == i)
                  {
                ...
                {
                lines[counter][0].color = sf::Color(200,0,0);
                lines[counter][1].color = sf::Color(200,0,0); 
                }
